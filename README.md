...
- **CLE課題の詳細確認**: 期限が近く未完了の課題を優先的にチェックします。採点済みは7日、提出済みは6時間、期限切れは24時間を目安に再確認します。未確認の課題が残る間はキャッシュ全体を完了扱いにしません。
- **CLE連絡事項の段階取得**: 現在履修中の科目だけを対象に、表示中または最近利用した科目を優先して1回最大4科目ずつ取得します。科目ごとに2時間のキャッシュを持ち、未取得科目が残る間は次回更新で巡回を続け、取得失敗時は以前のデータを保持します。
- **掲示同期の制限**: 掲示板のスナップショット同期は、ユーザーが **[掲示を同期]** ボタンを押したときのみ実行されます。初回は一覧を取得し、2回目以降はジャンルごとに既知の掲示だけのページが2ページ続いた時点でページ送りを停止します。ページ上限に到達した場合は完了扱いにせず、取得済みの新着だけを保存します。
