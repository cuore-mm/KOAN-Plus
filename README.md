...
- **CLE連絡事項の段階取得**: 現在履修中の科目だけを対象に、表示中または最近利用した科目を優先して1回最大4科目ずつ取得します。科目ごとに2時間のキャッシュを持ち、取得失敗時は以前のデータを保持します。
- **掲示同期の制限**: 掲示板のスナップショット同期は、ユーザーが **[掲示を同期]** ボタンを押したときのみ実行されます。初回は一覧を取得し、2回目以降はジャンルごとに既知の掲示へ到達した時点でページ送りを停止します。
- **掲示本文の不保持**: 掲示の詳細を開くと未読状態が変わるため、本文の自動事前取得（プリフェッチ）は行いません。
- **表示と再検証の分離**: キャッシュが期限切れでも保存済みデータを先に表示し、更新結果が返ってから差し替えます。同一URLの実行中リクエストは共有し、失敗が続く場合は対象ごとに指数バックオフを適用します。
